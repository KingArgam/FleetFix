...
import { Truck, MaintenanceEntry, Part, DowntimeEvent } from '../../types';
...
const TruckDetailView: React.FC<TruckDetailViewProps> = () => {
  ...
  const loadTruckData = async () => {
    ...
    try {
      // Find truck in state
      const foundTruck = state.trucks?.find(t => t.id === truckId);
      if (foundTruck) {
        setTruck(foundTruck);
      }
  ...
  const handleDeleteTruck = async () => {
    ...
    if (window.confirm(`Are you sure you want to delete ${truck.licensePlate}? This action cannot be undone.`)) {
      try {
        ...
        console.log('Delete truck:', truck.id);
        navigate('/trucks');
      } catch (error) {
      ...
      <div className="detail-header">
        ...
        <div className="header-content">
          ...
          <div className="header-actions">
            <button onClick={handleEditTruck} className="btn-secondary">
              <Edit size={16} />
              Edit Truck
            </button>
            <button onClick={handleDeleteTruck} className="btn-danger">
              <Trash2 size={16} />
              Delete
            </button>
          </div>
