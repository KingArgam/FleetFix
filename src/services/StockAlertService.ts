...
export class StockAlertService {
  ...
  async checkStockLevels(): Promise<void> {
    try {
      ...
      // Check each part for stock issues
  ...
  private async checkPartStockLevel(part: Part): Promise<void> {
    try {
      ...
      const reorderPoint = Math.max(minStock, Math.ceil(minStock * 1.2)); // 20% buffer
      ...
      if (alertType) {
        ...
        // No alert needed, deactivate any existing alerts for this part
        const existingAlertsQuery = query(
          collection(db, this.collectionName),
          where('partId', '==', part.id),
          where('isActive', '==', true)
  ...
  private calculateReorderQuantity(part: Part): number {
    ...
    // Calculate based on usage patterns (simplified)
    // In a real implementation, you'd analyze historical usage
  ...
  async startStockMonitoring(): Promise<() => void> {
    // Set up real-time monitoring of parts collection
    ...
    const unsubscribe = onSnapshot(partsQuery, async (snapshot) => {
      ...
      // Check stock levels for changed parts
      for (const part of changedParts) {
        await this.checkPartStockLevel(part);
      }
    });

    // Also run periodic checks (every hour)
